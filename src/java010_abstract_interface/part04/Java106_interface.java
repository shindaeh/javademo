...
//8. 객체 생성은 하지 못하지만, 참조형 타입으로서는 사용 가능하다.
				  // 변수에 final이 들어가면 상수
				  // 상수는 변경 불가능
				  // 상수는 int num = 10; - 무조건 정의를 해야한다.
                  //   ex string name = '6'; - 같은 예시로 상수는 무조건 값을 할당해야 한다.
...
public class Java106_interface {
	...
	public static void main(String[] args) {
		//interfac.variable(변수) - 호출 가능 (static이 붙어있기 때문)
		System.out.println(CarRun.carCount);
		
		
		//interface.method() - 호출 가능
		CarRun.display();
		...
		//interface에서 static 선언된 메소드는 객체참조변수,method()호출 할 수 없다.
		//truck.display();	
