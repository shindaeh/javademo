...
public static void main(String[] args) {
	...
	//2번에 해당하는 값만 받기 때문에 불가능 (조건 안됨_ 조건 제한에 걸림)
	...
	//Course.registerCourse2(new Applicant<Worker>(new Worker())); //적용불가능
	
	//직장인 및 일반인 등록 가능
	Course.registerCourse3(new Applicant<Worker>(new Worker()));
	Course.registerCourse3(new Applicant<Person>(new Worker()));
	//3번에 해당하는 값만 받는거기 때문에 불가능
	...
	//Course.registerCourse3(new Applicant<Student>(new Student())); //적용불가능
}// end main()
